...
interface SidebarProps {
  ...
  /** 当前视图，用于在视图切换时刷新最近任务列表 */
  currentView?: 'home' | 'workspace';
}

export const Sidebar: React.FC<SidebarProps> = ({ onTaskRestore, currentView }) => {
  // 获取最近任务（从任务存储中获取）
  const [recentTasks, setRecentTasks] = useState<Task[]>(getAllTasks().slice(0, 10));

  // 视图切换时刷新最近任务列表，确保显示最新状态
  useEffect(() => {
  ...
  const handleOpenProductIntro = () => {
    window.open('/product-intro/index.html', '_blank', 'noopener,noreferrer');
  };
    ...
    <div className="w-64 h-full bg-gray-50 border-r border-gray-200 flex flex-col">
      ...
      <div className="border-t border-gray-200 p-4 space-y-3">
        <div className="space-y-2">
          ...
          <button
            type="button"
            onClick={handleOpenProductIntro}
            className="flex w-full items-center gap-2 text-left text-xs text-gray-600 transition-colors hover:text-gray-800"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
